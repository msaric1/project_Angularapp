...
export class EventService{
    ...
    saveEvent(name : string, date : Date, time : string, price : number, location: {address: string, country: string, city: string}, onlineUrl : string, imageUrl : string){
        var newEvent : IEvent = {
    ...
    updateEvent(event : IEvent){
      //treba naci koji event treba updateati
      let idx = this.getEvent(event.id).id;
         EVENTS[idx] = event;
    }
    ...
    searchSessions(searchTerm: string){
        ...
        EVENTS.forEach(function(event){
           results= results.concat(event.sessions.filter(function(session){
               session.eventId = event.id; //session["eventId"] = event.id; pa ne treba ici u model oddatvati taj atribut
...
const EVENTS : IEvent [] = [
        {
            id: 1,
            name: "Client Web Programming",
            date: new Date("4/13/2017"),
            time: "10:00 am",
            price: 199.99,
            imageUrl: 'app/assets/images/angularconnect-shield.png',
            location: {
                address: "Trg Lj. Gaja 6",
                city: "Osijek",
                country: "Croatia"
            },
            sessions: [
